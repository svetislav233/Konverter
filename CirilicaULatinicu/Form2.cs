...
namespace CirilicaULatinicu
{
    public partial class Form2 : Form
    {
        ...
        private string GetTextFromPDF()
        {
            ...
            using (PdfReader reader = new PdfReader(putanjaDoFajla))
            {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            snimi.Filter = "Text File|*.txt";
            if (snimi.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                nekaPutanja = snimi.FileName;
                StreamWriter snimac = new StreamWriter(File.Create(nekaPutanja));
                snimac.Write(textBox1.Text);
                snimac.Dispose();
                MessageBox.Show("Uspesno snimanje txt datoteke");
            }
        }

        private void otvoriPDFToolStripMenuItem_Click(object sender, EventArgs e)
        {
            OpenFileDialog fajl = new OpenFileDialog();
            if (fajl.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                putanjaDoFajla = fajl.FileName;
            }
            try
            {
                string tekst = GetTextFromPDF();
                textBox1.Text = tekst;
            }
            catch
            {
        ...
        private void otvoritxtFajlToolStripMenuItem_Click(object sender, EventArgs e)
        {
            OpenFileDialog fajl = new OpenFileDialog();
            if (fajl.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
            ...
            try
            {
                string text = System.IO.File.ReadAllText(@putanjaDoFajla);
                textBox1.Text = text;
